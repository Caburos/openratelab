...
- [x] Direct-answer structure on key pages (homepage, service pages, top blog posts) — **audited 2026-08-21**. Checked homepage hero/services/FAQ and every H2 in the 3 cornerstone blog posts (performance report, flows guide, benchmarks): the large majority already lead with a direct answer in the first sentence (established writing pattern, not new). Found and fixed 2 genuinely weak openings on `klaviyo-performance-report-2026.html` ("What Does the Unsubscribe Rate Look Like?" and "How Do These Numbers Compare to Industry Averages?") that deferred the number to a table/callout below instead of leading with it — both fixed using data already published elsewhere on the same page, no new numbers introduced.
- [x] Sourced stats + named expert quotes where possible (+25.9% / +27.8% citation lift per 2026 GEO research) — **done, 2026-08-21**. Case-study stats were already sourced (pulled directly from Klaviyo reporting per each page's DATA_NOTE). The "named expert quote" gap was real: 11 of 12 case studies attributed their "Why This Worked" analysis to a generic "OPENRATELAB ANALYSIS" label. Replaced with "UROS KORENE, FOUNDER" — he's already the site's verified author in JSON-LD and the About page bio, so this surfaces an existing true fact rather than adding a new claim.
- [x] Extend FAQPage + Article + BreadcrumbList "triple stack" schema to every blog/case-study page, not just the homepage — **done, 2026-08-21**. Audited all 23 blog/case-study pages: all 11 blog posts already had the full triple stack. Case studies only had Article + BreadcrumbList (no FAQPage) — 3 already had a visible FAQ accordion to source schema from (`activewear-brand-email-engagement`, `ecommerce-open-rate-system`, `health-supplement-brand-at-scale`); added matching FAQPage schema, verified byte-for-byte against the visible text. The other 9 had no FAQ section at all — per Uros's go-ahead, drafted a 2-question FAQ (visible accordion + matching schema) for each, sourced only from facts already published on that specific case study's own page (revenue figures, flow counts, program length, client quotes already on the page) — no new claims introduced. All 12 case studies now carry the full triple stack. Also fixed an unrelated pre-existing British spelling ("OPTIMISED" → "OPTIMIZED") found in passing on `nutrition-brand-69k-revenue.html`.
- [~] **Entity consistency**: "OpenRateLab" name/description/founder name identical across the site, Google Business Profile, LinkedIn, and Clutch — **partial, 2026-08-21**. Internal site check is clean: "OpenRateLab" and "Uros Korene" spelled identically across every JSON-LD block, the About page bio, and all visible bylines/attributions — no variants found. Google Business Profile and LinkedIn don't exist yet (both intentionally skipped earlier — Phase 1 and Phase 5). Re-checking against the live Clutch profile needs an active vendor-dashboard session (it's not a public URL I could safely guess); that re-check is pending Uros opening the Clutch dashboard in a connected browser session.
- [x] `llms.txt`: **done, 2026-08-21**. Added a minimal `llms.txt` at the repo root per the "cheap to add" guidance — one-paragraph agency summary plus links to every real published page (homepage, about, blog index, all 11 blog posts, all 12 case studies). No new claims, just a structured index of what already exists. Not over-invested in beyond this, per the low-priority note below.
- [~] Monthly manual citation check: ask ChatGPT/Perplexity/Gemini/Copilot "best Klaviyo email agency for DTC brands" (and similar) and log whether OpenRateLab appears — **attempted, 2026-08-21**. Tried to run the first check live via Perplexity in a browser session; the query wouldn't submit through browser automation after a privacy-dialog dismissal and a retry (likely anonymous-session friction, not a bot-detection wall worth pushing through). Not force-attempted further. This check needs to be run manually by Uros (or from a logged-in session) going forward — it's a 5-minute monthly task: ask the query in ChatGPT, Perplexity, Gemini, and Copilot, and note whether openratelab.com is cited.

### QA — Phase 4, 2026-08-21
Ran `/code-review` (high effort) plus an independent fact-verification pass across all Phase 4 commits (`be57026..9aaf42c`).
- **Fact-check**: dispatched a separate read-only agent to re-verify all 9 new case-study FAQs against the full source page (not just the extracted snippets used to draft them) — every number and claim traced cleanly to the same page's hero/Delta/DATA_NOTE content, including one case (`farm-direct-pet-treats-brand.html`) where the FAQ correctly preserved the page's own "not yet validated long-term" caveat rather than overselling it.
- **Structural**: all 22 touched JSON-LD blocks across 12 case studies re-validated as parseable; `sitemap.xml` re-validated as well-formed XML with 26 entries and zero duplicates; every URL in the new `llms.txt` confirmed to resolve to a real file.
- **Live verification**: confirmed `llms.txt`, the named-attribution change, and the direct-answer fixes are all live on production post-deploy.
- No findings — nothing needed fixing this pass.
